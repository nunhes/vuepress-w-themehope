...
Ter en conta que usamos o [operador ternario](https://www.javascripttutorial.net/javascript-ternary-operator/) `?:` para executar o método `complete()`  ou  `start()` en función do valor da propiedade `this.timeRemaining`.
...
O seguinte define o método `update()` que chama a devolución `onRender()` de chamada co obxecto de tempo restante actual devolto polo método `getTime()`:
